...
bool DFGNode::isVectorized() {
  // Check if the instruction's result type is a vector
  Value* psVal = cast<Value>(m_inst);
  if (psVal->getType()->isVectorTy()) {
    return true;
  }

  // For store instructions, check if the value being stored is a vector
  if (StoreInst* storeInst = dyn_cast<StoreInst>(m_inst)) {
    return storeInst->getValueOperand()->getType()->isVectorTy();
  }

  // For other instructions, check if any operand is a vector type
  // This catches cases where the result is void but operands are vectors
  for (unsigned i = 0; i < m_inst->getNumOperands(); ++i) {
    if (m_inst->getOperand(i)->getType()->isVectorTy()) {
      return true;
    }
  }
