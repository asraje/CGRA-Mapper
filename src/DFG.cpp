...
void DFG::generateJSON() {
  ...
  for (DFGNode* node: nodes) {
    ...
    jsonFile<<"    \"vectorized\" : "<<(node->isVectorized() ? "true" : "false")<<",\n";
    jsonFile<<"    \"tile_type\"  : \""<<(node->isVectorized() ? "vector" : "scalar")<<"\",\n";
    jsonFile<<"    \"in_const\"   : [";
...
// Helper function to sanitize node names for DOT format (replace dots with underscores)
...
//    if (dyn_cast<Instruction>((*node)->getInst())) {
    if (t_isTrimmedDemo) {
      string safeName = sanitizeForDot(node->getOpcodeName());
      string tileType = node->isVectorized() ? "[VEC]" : "[SCL]";
      file << "\tNode" << node->getID() << safeName << "[shape=record, label=\"" << "(" << node->getID() << ") " << node->getOpcodeName() << "_" << node->getBBID() << " " << tileType << "\"];\n";
    } else {
  ...
  */
  ...
  for (DFGEdge* edge: m_ctrlEdges) {
    ...
    if (find(m_DFGEdges.begin(), m_DFGEdges.end(), edge) != m_DFGEdges.end()) {
      if (t_isTrimmedDemo) {
        string srcName = sanitizeForDot(edge->getSrc()->getOpcodeName());
        string dstName = sanitizeForDot(edge->getDst()->getOpcodeName());
        file << "\tNode" << edge->getSrc()->getID() << srcName << " -> Node" << edge->getDst()->getID() << dstName << "\n";
      } else {
