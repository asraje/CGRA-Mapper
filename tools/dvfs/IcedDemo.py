...
def DVFSComp(fileName, uFactor):
    ...
    if uFactor == 1:
        compileCommand = f"clang -emit-llvm -fno-unroll-loops -O3 -o kernel.bc -c {KERNEL_DIRECTORY}/{fileSource}/{fileName}"
    elif uFactor == 2:
        compileCommand = f"clang -emit-llvm -funroll-loops -mllvm -unroll-count={uFactor} -O3 -o kernel.bc -c {KERNEL_DIRECTORY}/{fileSource}/{fileName}"
    else:
    ...
    disassembleCommand = "llvm-dis kernel.bc -o kernel.ll"
...
def DVFSMap(kernel,df):
    ...
    Returns: NULL
    """
    getMapCommand = "opt -load-pass-plugin=../../build/src/libmapperPass.so -passes=mapperPass kernel.bc"
