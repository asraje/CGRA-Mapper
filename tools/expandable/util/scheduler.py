...
class Kernel:
    ...
    def comp_kernel(self):
        ...
        if self.unroll_factor == 1 and self.vector_factor == 1:
            compile_command = f"clang -emit-llvm -fno-unroll-loops -fno-vectorize -O3 -o kernel.bc -c {KERNEL_DIRECTORY}/{file_source}/{self.kernel_name}"
        elif self.unroll_factor == 1 and self.vector_factor != 1:
            compile_command = f"clang -emit-llvm -fno-unroll-loops -O3 -mllvm -force-vector-width={self.vector_factor} -o kernel.bc -c {KERNEL_DIRECTORY}/{file_source}/{self.kernel_name}"
        elif self.unroll_factor != 1 and self.vector_factor == 1:
            compile_command = f"clang -emit-llvm -funroll-loops -mllvm -unroll-count={self.unroll_factor} -fno-vectorize -O3 -o kernel.bc -c {KERNEL_DIRECTORY}/{file_source}/{self.kernel_name}"
        else:
        ...
        disassemble_command = f"llvm-dis kernel.bc -o kernel.ll"
    ...
    def map_kernel(self):
        ...
        Returns: NULL
        """
        get_map_command = f"opt -load-pass-plugin=../../build/src/libmapperPass.so -passes=mapperPass kernel.bc"
